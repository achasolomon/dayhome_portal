...
function AcceptInviteForm() {
  ...
  async function onSubmit(data: AcceptInviteForm) {
    try {
      ...
      setSuccess(true);
      const result = await authApi.login(inviteEmail, data.password);
      const orgType = result.user.organizationType;
      const role = result.user.role;
      if (orgType === 'system' || ['SUPER_ADMIN', 'ORG_ADMIN', 'ORG_MANAGER', 'BILLING_ONLY'].includes(role)) {
        router.push('/admin/dashboard');
      } else if (orgType === 'dayhome' || ['DAYHOME_OWNER', 'EDUCATOR'].includes(role)) {
        router.push('/dayhome/dashboard');
      } else {
